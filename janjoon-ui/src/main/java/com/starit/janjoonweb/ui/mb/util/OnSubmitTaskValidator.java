...
public class OnSubmitTaskValidator implements Validator {
	...
	public void validate(FacesContext arg0, UIComponent component, Object value) throws ValidatorException {
		...
		if (type.equalsIgnoreCase("workload")) {
			...
			if (workload == null) {
				FacesMessage facesMessage = MessageFactory.getMessage("validator_task_workloadRequired", "Task");
				facesMessage.setSeverity(FacesMessage.SEVERITY_ERROR);
				throw new ValidatorException(facesMessage);

			}
			if (workload <= 0) {
				FacesMessage facesMessage = MessageFactory.getMessage("validator_task_workloadNegatif", "Task");
				facesMessage.setSeverity(FacesMessage.SEVERITY_ERROR);
				throw new ValidatorException(facesMessage);
			}
		...
		if (type.equalsIgnoreCase("startDate")) {
			...
			if (startDate == null) {
				FacesMessage facesMessage = MessageFactory.getMessage("validator_task_startDateRequired", "Task");
				facesMessage.setSeverity(FacesMessage.SEVERITY_ERROR);
				throw new ValidatorException(facesMessage);
				...
				if (((JJTaskBean) LoginBean.findBean("jJTaskBean")).getMode().equalsIgnoreCase("scrum")) {
					if (startDate.before(((JJSprintBean) LoginBean.findBean("jJSprintBean")).getSprintUtil().getSprint()
					        .getStartDate())) {
						FacesMessage facesMessage = MessageFactory.getMessage("validator_task_startBeforeSprintStart",
						        "Task");
						facesMessage.setSeverity(FacesMessage.SEVERITY_ERROR);
						throw new ValidatorException(facesMessage);
					} else if (startDate.after(((JJSprintBean) LoginBean.findBean("jJSprintBean")).getSprintUtil()
					        .getSprint().getEndDate())) {
						FacesMessage facesMessage = MessageFactory.getMessage("validator_task_endAfterSprintEnd",
						        "Task");
						facesMessage.setSeverity(FacesMessage.SEVERITY_ERROR);
						throw new ValidatorException(facesMessage);
					}
					...
					if (((JJTaskBean) LoginBean.findBean("jJTaskBean")).getSprint() != null) {
						if (startDate
						        .before(((JJTaskBean) LoginBean.findBean("jJTaskBean")).getSprint().getStartDate())) {
							FacesMessage facesMessage = MessageFactory
							        .getMessage("validator_task_startBeforeSprintStart", "Task");
							facesMessage.setSeverity(FacesMessage.SEVERITY_ERROR);
							throw new ValidatorException(facesMessage);
						} else if (startDate
						        .after(((JJTaskBean) LoginBean.findBean("jJTaskBean")).getSprint().getEndDate())) {
							FacesMessage facesMessage = MessageFactory.getMessage("validator_task_endAfterSprintEnd",
							        "Task");
							facesMessage.setSeverity(FacesMessage.SEVERITY_ERROR);
							throw new ValidatorException(facesMessage);
