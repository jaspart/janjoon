...
public class JJChapterBean {
	...
	private void transferTree() {
		...
		for (JJRequirement requirement : jJRequirementList) {
			TreeNode node = new DefaultTreeNode(
					"R-" + requirement.getId() + "- " + requirement.getName(),
					leftRoot);
		}
		...
		for (JJChapter chapter : parentChapters) {
			TreeNode node = createTree(chapter, rightRoot, project, category,
					1);
		}
	...
	private List<String> getChildren(JJChapter parent) {
		...
		List<JJChapter> children = jJChapterService
				.getChildrenOfParentChapter(parent, true, false);
	...
	public TreeNode createTree(JJChapter chapterParent, TreeNode rootNode,
		...
		TreeNode newNode = new DefaultTreeNode(
				"C-" + chapterParent.getId() + "- " + chapterParent.getName(),
				rootNode);
			...
			for (Map.Entry<Integer, Object> entry : elements.entrySet()) {
				...
				if (className.equalsIgnoreCase("JJChapter")) {
					...
					JJRequirement requirement = (JJRequirement) entry
							.getValue();
					TreeNode newNode3 = new DefaultTreeNode(
							"R-" + requirement.getId() + "- "
									+ requirement.getName(),
							newNode);
				}
	...
	public StreamedContent getPreProcessPDF()
		...
		if (withOutChapter != null && !withOutChapter.isEmpty()) {
			paragraph.add(new Chunk("\n " + MessageFactory
					.getMessage("specification_tree_withOutChapter", "")
					.getDetail() + "\n", fontChapter));

			for (JJRequirement requirement : withOutChapter) {
				paragraph.add(new Chunk(MessageFactory
	...
	private SortedMap<Integer, JJTestcase> getSortedTestcases(
		...
		List<JJTestcase> testcases = jJTestcaseService.getTestcases(requirement,
				chapter, LoginBean.getVersion(), null, false, true, false);
		...
		if (dragNodeData.startsWith("R-")) {
			...
			if (dropNodeData.startsWith("C-")) {
				...
				if (requirementCHAPTER == null) {
					...
					message = MessageFactory
							.getMessage("chapter_successfully_detached",
									FacesMessage.SEVERITY_WARN, dragNodeData,
									dropNodeData,
									MessageFactory
			...
			if (dropNodeData.startsWith("C-")
				...
				if (dropNodeData.equalsIgnoreCase("rightRoot")) {
					...
					message = MessageFactory
							.getMessage("chapter_parent_document",
									FacesMessage.SEVERITY_WARN, dragNodeData,
									dropNodeData,
									MessageFactory
											.getMessage("label_chapter", "")
											.getDetail());
				} else {
					...
					message = MessageFactory
							.getMessage("chapter_successfully_droped",
				...
				SortedMap<Integer, Object> tmpElements = getSortedElements(null,
						project, category, false);
