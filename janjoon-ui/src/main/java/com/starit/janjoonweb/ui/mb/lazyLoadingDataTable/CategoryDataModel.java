...
public class CategoryDataModel extends LazyDataModel<JJRequirement> {
		...
		if (categoryId != 0) {
			...
			if (jJStatusBean != null
				...
				for (int i = 0; i < jJStatusBean.getCategoryDataModel()
						.size(); i++) {
					if (jJStatusBean.getCategoryDataModel().get(i).getCategory()
							.equals(category)
			...
			if (!containCategory) {
				...
				boolean isHighCategory = jJCategoryService.isHighLevel(category,
						LoginBean.getCompany());

				for (JJRequirement requirement : allRequirements) {
					{
						requirement = JJRequirementBean.getRowState(requirement,
								jJRequirementService);
						if (requirement.getState().getName().equalsIgnoreCase(
			...
			if (jJStatusBean != null
				...
				for (int i = 0; i < jJStatusBean.getCategoryDataModel()
					...
					if (jJStatusBean.getCategoryDataModel().get(i).getCategory()
						...
						completionProgress = jJStatusBean.getCategoryDataModel()
								.get(i).getCompletionProgress();
						containCategory = true;
						i = jJStatusBean.getCategoryDataModel().size();
					}
	...
	private float calculCompletion(JJRequirement r) {
		...
		if (JJRequirementBean.getRowState(r, jJRequirementService).getState()
				...
				|| JJRequirementBean.getRowState(r, jJRequirementService)
	...
	public boolean checkChapter(JJRequirement req) {
		...
		boolean check = !filterChapter
				...
				|| (selectedChapter != null && req.getChapter() != null
						&& selectedChapter.equals(req.getChapter()));

		if (check)
			return check;
		else {
			List<JJChapter> list = jJChapterService
					.getChildrenOfParentChapter(selectedChapter, true, true);

			int i = 0;
			while (!check && i < list.size()) {
				check = !filterChapter
						|| (list.get(i) != null && req.getChapter() != null
								&& list.get(i).equals(req.getChapter()));
				i++;
			}
	...
	public boolean equals(Object object) {
		return (object instanceof CategoryDataModel) && (categoryId != 0)
				? categoryId == ((CategoryDataModel) object).getCategoryId()
				: (object == this);
	}
	...
	public List<JJRequirement> load(int first, int pageSize,
		...
		for (JJRequirement req : allRequirements) {
			...
			match = match && (mineContact == null
					|| (req.getUpdatedBy() != null
							&& req.getUpdatedBy().equals(mineContact))
					|| (req.getCreatedBy() != null
							&& req.getCreatedBy().equals(mineContact)));

			match = match && (viewLinkRequirement == null
					|| (viewLinkRequirement.equals(req))
					|| (viewLinkRequirement.getRequirementLinkDown()
							.contains(req)
							|| viewLinkRequirement.getRequirementLinkUp()
									.contains(req)));
