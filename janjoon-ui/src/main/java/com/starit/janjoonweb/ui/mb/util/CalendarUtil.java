...
public class CalendarUtil {
	...
	public void initCompanyCalendar() throws IOException {
		...
		while (i < 7) {
			...
			if (workDay == null || workDay.contains("null")) {
				...
				try {
					staDate1 = new SimpleDateFormat(hour_format)
				...
				try {
					enDate1 = new SimpleDateFormat(hour_format)
							.parse(workDay.substring(6, 11));
				} catch (ParseException e) {
					enDate1 = null;
				}
				try {
					staDate2 = new SimpleDateFormat(hour_format)
							.parse(workDay.substring(14, 19));
				} catch (ParseException | StringIndexOutOfBoundsException e) {
					staDate2 = null;
				}
				try {
					endDate2 = new SimpleDateFormat(hour_format)
		...
		for (Object key : Collections.list(properties.keys())) {
			if (key.toString().contains(HOLIDAYS)) {
				String hol = properties.getProperty((String) key).toString()
						.trim() + year;
				try {
					holidays.add(getZeroTimeDate(
							new SimpleDateFormat(day_format).parse(hol)));
				} catch (ParseException e) {
	...
	public static String addHoliday(Date date, String calendar, int size)
		...
		properties.setProperty(HOLIDAYS + "." + String.format("%01d", size + 1),
				f.format(date));
		...
		for (Object key : Collections.list(properties.keys())) {
			if (key.toString().contains(HOLIDAYS)
					&& properties.getProperty((String) key)
							.equalsIgnoreCase(f.format(date))) {
