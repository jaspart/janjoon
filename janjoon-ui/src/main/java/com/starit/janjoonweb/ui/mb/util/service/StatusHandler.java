...
public class StatusHandler implements ISVNStatusHandler, ISVNEventHandler {
	...
	public void handleStatus(SVNStatus status) {
		...
		if (status.getRemotePropertiesStatus() != SVNStatusType.STATUS_NONE
				|| status
						.getRemoteContentsStatus() != SVNStatusType.STATUS_NONE) {
			/*
		...
		offsets[0] = offset.substring(0,
				6 - String.valueOf(workingRevision).length());
		offsets[1] = offset.substring(0,
				6 - String.valueOf(lastChangedRevision).length());
		// status
		offsets[2] = offset.substring(0,
				offset.length() - (status.getAuthor() != null
						? status.getAuthor().length()
						: 1));
		/*
		...
		System.out.println(pathChangeType + propertiesChangeType
				...
				+ (lastChangedRevision >= 0
						? String.valueOf(lastChangedRevision)
						: "?")
				+ offsets[1]
	...
	public void handleEvent(SVNEvent event, double progress) {
		...
		if (action == SVNEventAction.STATUS_COMPLETED) {
			System.out.println(
					"Status against revision:  " + event.getRevision());
		}
