...
public class JJStatusBean {
	...
	public PieChartModel getPieChart() {
		...
		if (pieChart == null) {
			...
			List<JJStatus> statReq = jJStatusService.getStatus("Requirement", true, null, false);
			boolean render = false;
			for (JJStatus s : statReq) {

				int i = Integer.parseInt("" + jJRequirementService.getReqCountByStaus(
				        ((LoginBean) LoginBean.findBean("loginBean")).getContact().getCompany(), project,
				        LoginBean.getProduct(), LoginBean.getVersion(), s, true));
				render = render || i > 0;
				if (i > 0)
					pieChart.set(MessageFactory.getMessage("status_" + s.getName(), "").getDetail(), i);
			}

			if (render) {
				pieChart.setLegendPosition("e");
				pieChart.setTitle("% " + MessageFactory.getMessage("label_requirement", "").getDetail());
	...
	public PieChartModel getBugPieChart() {
		...
		if (activeTabIndex == 1) {
			if (bugPieChart == null) {
				...
				List<JJStatus> statBug = jJStatusService.getStatus("Bug", true, null, false);
				boolean render = false;
				for (JJStatus s : statBug) {

					int i = Integer.parseInt("" + jJBugService.getBugsCountByStaus(
					        ((LoginBean) LoginBean.findBean("loginBean")).getContact().getCompany(), project,
					        LoginBean.getProduct(), LoginBean.getVersion(), s, true));
					render = render || i > 0;
					if (i > 0)
						bugPieChart.set(MessageFactory.getMessage("status_" + s.getName(), "").getDetail(), i);
				}
	...
	public MeterGaugeChartModel getBugMetergauge() {
		if (activeTabIndex == 1) {
			if (bugMetergauge == null) {
				...
				List<JJRequirement> requirements = jJRequirementService.getRequirements(null,
				        ((LoginBean) LoginBean.findBean("loginBean")).getAuthorizedMap("Requirement",
				                LoginBean.getProject(), LoginBean.getProduct()),
				        LoginBean.getVersion());

				for (JJRequirement req : requirements) {

					bugKPI = bugKPI + (1 / (1 + jJBugService.requirementBugCount(req)));
				}
	...
	public boolean isEmptysprintList() {
		if (emptysprintList == null)
			emptysprintList = jJSprintService.getSprints(LoginBean.getProject(), true).isEmpty();
			...
			if (activeTabIndex == 2 && activeTabSprintIndex == -1) {
				...
				while (i < getSprintList().size()) {
					if (!getSprintList().get(i).isRender()
					        && now.after(getSprintList().get(i).getSprint().getStartDate())
					        && now.before(getSprintList().get(i).getSprint().getEndDate())) {
				...
				if (activeTabSprintIndex == -1) {
					...
					for (SprintUtil s : getSprintList()) {
						if (!s.isRender()) {
							long diff = Math.abs(currentTime - s.getSprint().getEndDate().getTime());
	...
	public void loadData() {
		...
		if (project == null) {
			...
			if (project != null) {
				...
				List<JJCategory> categoryList = jJCategoryService.getCategories(null, false, true, true,
				...
				List<JJRequirement> requirements = jJRequirementService.getRequirements(null,
				        loginBean.getAuthorizedMap("Requirement", LoginBean.getProject(), LoginBean.getProduct()),
				        LoginBean.getVersion());
				...
				if (project.getStartDate() != null && project.getEndDate() != null) {
					float tmpsProj = (new Date().getTime() - project.getStartDate().getTime());
					float tmps = (project.getEndDate().getTime() - project.getStartDate().getTime());
				...
				prjMetergauge.setTitle(MessageFactory.getMessage("label_project", "").getDetail() + " KPI");
	...
	public void deleteStatus() {
		...
		FacesMessage facesMessage = MessageFactory.getMessage("message_successfully_deleted", "Status", "");
	...
	public void persistStatus() {
		...
		if (getJJStatus_().getId() != null) {
			...
			getJJStatus_().setDescription(
			        "Satus : " + getJJStatus_().getName() + " for " + getJJStatus_().getObjet() + "object");
	...
	private LineChartModel initLinearModel() {
		...
		List<JJSprint> sprints = jJSprintService.getSprints(LoginBean.getProject(), true);

		ChartSeries blueSeries = new ChartSeries();
		blueSeries.setLabel(MessageFactory.getMessage("statistique_help_bug_blue", "").getDetail());

		ChartSeries purpleSerie = new ChartSeries();
		purpleSerie.setLabel(MessageFactory.getMessage("statistique_help_bug_purple", "").getDetail());

		ChartSeries greenSerie = new ChartSeries();
		greenSerie.setLabel(MessageFactory.getMessage("statistique_help_bug_green", "").getDetail());

		for (JJSprint sp : sprints) {
			List<JJTask> tasks = jJTaskService.getTasksByObject(sp, LoginBean.getProject(), LoginBean.getProduct(),
			        "bug", true);
			...
			for (JJTask ta : tasks) {
				...
				moyPurple = moyPurple + endDate.getTime() / Hours_IN_MILLIS
				        - ta.getBug().getCreationDate().getTime() / Hours_IN_MILLIS;
			...
			tasks = jJTaskService.getTasksByObject(sp, LoginBean.getProject(), LoginBean.getProduct(), "requirement",
			        true);
			float moyGreen = 0;
			for (JJTask ta : tasks) {
	...
	public HtmlPanelGrid populateCreatePanel() {
		...
		nameCreateOutput.setValue(MessageFactory.getMessage("admin_status_table_name_title", "").getDetail() + ":");
		...
		nameCreateInput.setValueExpression("value",
		        expressionFactory.createValueExpression(elContext, "#{jJStatusBean.JJStatus_.name}", String.class));
		...
		AutoComplete objetCreateInput = (AutoComplete) application.createComponent(AutoComplete.COMPONENT_TYPE);
		objetCreateInput.setId("objetStatCreateInput");
		objetCreateInput.setValueExpression("value",
		        expressionFactory.createValueExpression(elContext, "#{jJStatusBean.JJStatus_.objet}", String.class));
		objetCreateInput.setCompleteMethod(expressionFactory.createMethodExpression(elContext,
		        "#{jJStatusBean.completeObject}", List.class, new Class[] { String.class }));
		objetCreateInput.setDropdown(true);
		objetCreateInput.setRequired(true);
		objetCreateInput.setScrollHeight(200);
		objetCreateInput.setValueExpression("var",
		        expressionFactory.createValueExpression(elContext, "objet", String.class));
		objetCreateInput.setValueExpression("itemLabel",
		        expressionFactory.createValueExpression(elContext, "#{objet}", String.class));
		objetCreateInput.setValueExpression("itemValue",
		        expressionFactory.createValueExpression(elContext, "#{objet}", String.class));
		...
		levelStatusCreateInput.setValueExpression("value", expressionFactory.createValueExpression(elContext,
		        "#{jJStatusBean.JJStatus_.levelStatus}", Integer.class));
	...
	public HtmlPanelGrid populateEditPanel() {
		...
		nameEditOutput.setValue(MessageFactory.getMessage("admin_status_table_name_title", "").getDetail() + ":");
		...
		nameEditInput.setValueExpression("value",
		        expressionFactory.createValueExpression(elContext, "#{jJStatusBean.JJStatus_.name}", String.class));
		...
		AutoComplete objetEditInput = (AutoComplete) application.createComponent(AutoComplete.COMPONENT_TYPE);
		objetEditInput.setId("objetStatEditInput");
		// objetEditInput.
		objetEditInput.setValueExpression("value",
		        expressionFactory.createValueExpression(elContext, "#{jJStatusBean.JJStatus_.objet}", String.class));
		objetEditInput.setRequired(true);
		objetEditInput.setCompleteMethod(expressionFactory.createMethodExpression(elContext,
		        "#{jJStatusBean.completeObject}", List.class, new Class[] { String.class }));
		objetEditInput.setDropdown(true);
		objetEditInput.setScrollHeight(200);
		objetEditInput.setValueExpression("var",
		        expressionFactory.createValueExpression(elContext, "objet", String.class));
		objetEditInput.setValueExpression("itemLabel",
		        expressionFactory.createValueExpression(elContext, "#{objet}", String.class));
		objetEditInput.setValueExpression("itemValue",
		        expressionFactory.createValueExpression(elContext, "#{objet}", String.class));
		objetEditInput.setRequired(true);
		...
		levelStatusEditInput.setValueExpression("value", expressionFactory.createValueExpression(elContext,
		        "#{jJStatusBean.JJStatus_.levelStatus}", Integer.class));
	...
	public HtmlPanelGrid populateViewPanel() {
		...
		nameLabel.setValue(MessageFactory.getMessage("admin_status_table_name_title", "").getDetail() + ":");
		...
		descriptionValue.setValueExpression("value", expressionFactory.createValueExpression(elContext,
		...
		creationDateValue.setValueExpression("value", expressionFactory.createValueExpression(elContext,
		...
		createdByValue.setValueExpression("value", expressionFactory.createValueExpression(elContext,
		        "#{jJStatusBean.JJStatus_.createdBy}", JJContact.class));
		...
		updatedDateValue.setValueExpression("value", expressionFactory.createValueExpression(elContext,
		...
		updatedByValue.setValueExpression("value", expressionFactory.createValueExpression(elContext,
		        "#{jJStatusBean.JJStatus_.updatedBy}", JJContact.class));
		...
		enabledValue.setValueExpression("value",
		        expressionFactory.createValueExpression(elContext, "#{jJStatusBean.JJStatus_.enabled}", String.class));
		...
		objetValue.setValueExpression("value",
		        expressionFactory.createValueExpression(elContext, "#{jJStatusBean.JJStatus_.objet}", String.class));
		...
		levelStatusValue.setValueExpression("value", expressionFactory.createValueExpression(elContext,
	...
	public class CategoryDataModel {
		...
		public float calculateCoverageProgress() {
			...
			if (coverageProgress == -1) {
				...
				if (jJRequirementBean != null && jJRequirementBean.getTableDataModelList() != null
					...
					for (int i = 0; i < jJRequirementBean.getTableDataModelList().size(); i++) {
						if (jJRequirementBean.getTableDataModelList().get(i).getCategoryId() != 0 && category.getId()
						        .equals(jJRequirementBean.getTableDataModelList().get(i).getCategoryId())) {
							if (jJRequirementBean.getTableDataModelList().get(i).getCoverageProgress() == -1)
								jJRequirementBean.getTableDataModelList().get(i).calculCoverageProgress();
							coverageProgress = jJRequirementBean.getTableDataModelList().get(i).getCoverageProgress()
							        / 100;
							containCategory = true;
							i = jJRequirementBean.getTableDataModelList().size();
						}
					}
				}
				if (!containCategory) {
					float compteur = 0;
					LoginBean loginBean = (LoginBean) LoginBean.findBean("loginBean");
					List<JJRequirement> dataList = jJRequirementService.getRequirements(
					        ((LoginBean) LoginBean.findBean("loginBean")).getContact().getCompany(), category,
					        loginBean.getAuthorizedMap("Requirement", project, LoginBean.getProduct()),
					        LoginBean.getVersion(), null, null, false, true, false, false, null);

					boolean isLowCategory = jJCategoryService.isLowLevel(category, LoginBean.getCompany());
					boolean isHighCategory = jJCategoryService.isHighLevel(category, LoginBean.getCompany());

					for (JJRequirement requirement : dataList) {
						{
							requirement = JJRequirementBean.getRowState(requirement, jJRequirementService);
							if (requirement.getState().getName()
							        .equalsIgnoreCase(JJRequirementBean.jJRequirement_InProgress)
							        || requirement.getState().getName()
							                .equalsIgnoreCase(JJRequirementBean.jJRequirement_InTesting)
							        || requirement.getState().getName()
							                .equalsIgnoreCase(JJRequirementBean.jJRequirement_Finished)) {
								compteur++;
							} else if (isLowCategory && requirement.getState().getName()
							        .equalsIgnoreCase(JJRequirementBean.jJRequirement_Specified)) {
								compteur++;

							} else if (isHighCategory && (requirement.getState().getName()
							        .equalsIgnoreCase(JJRequirementBean.jJRequirement_Specified)
							        || jJTaskService.haveTask(requirement, true, false, false))) {

								compteur += 0.5;

							} else if (!isHighCategory && !isLowCategory && requirement.getState().getName()
							        .equalsIgnoreCase(JJRequirementBean.jJRequirement_Specified)) {
								compteur++;
							} else if (!isHighCategory && !isLowCategory
							        && (jJRequirementService.haveLinkUp(requirement)
							                || jJRequirementService.haveLinkDown(requirement))) {
								compteur += 0.5;
							}
						}
					}
			...
			if (completionProgress == -1) {
				...
				if (jJRequirementBean != null && jJRequirementBean.getTableDataModelList() != null
					...
					for (int i = 0; i < jJRequirementBean.getTableDataModelList().size(); i++) {
						if (jJRequirementBean.getTableDataModelList().get(i).getCategoryId() != 0 && category.getId()
						        .equals(jJRequirementBean.getTableDataModelList().get(i).getCategoryId())) {
							if (jJRequirementBean.getTableDataModelList().get(i).getCompletionProgress() == -1)
								jJRequirementBean.getTableDataModelList().get(i).calculCompletionProgress();
							completionProgress = jJRequirementBean.getTableDataModelList().get(i)
		...
		private float calculCompletion(JJRequirement requirement) {
			...
			if (JJRequirementBean.getRowState(requirement, jJRequirementService).getState().getName()
			        .equalsIgnoreCase(JJRequirementBean.jJRequirement_InProgress)
			        || JJRequirementBean.getRowState(requirement, jJRequirementService).getState().getName()
			                .equalsIgnoreCase(JJRequirementBean.jJRequirement_InTesting)
			        || JJRequirementBean.getRowState(requirement, jJRequirementService).getState().getName()
			                .equalsIgnoreCase(JJRequirementBean.jJRequirement_Finished)) {
				compteur++;
		...
		public boolean equals(Object obj) {
			...
			return (obj instanceof CategoryDataModel) && (getCategory() != null)
			        ? getCategory().equals(((CategoryDataModel) obj).getCategory()) : (obj == this);
		}
