...
public class AutoCompleteRenderer extends
	...
	protected void encodeInput(FacesContext context, AutoComplete ac,
		...
		String itemLabel;
		String defaultStyleClass = ac.isDropdown() ? AutoComplete.INPUT_WITH_DROPDOWN_CLASS
				: AutoComplete.INPUT_CLASS;
		String styleClass = disabled ? defaultStyleClass + " ui-state-disabled"
				: defaultStyleClass;
		...
		inputStyleClass = (inputStyleClass == null) ? styleClass : styleClass
				+ " " + inputStyleClass;
		...
		if (var == null) {
			...
			if (ac.isValid()) {
				...
				itemLabel = (submittedValue == null) ? null : String
		...
		if (ac.isReadonly()
				|| (LoginBean.findBean("loginBean") != null && ((LoginBean) LoginBean
