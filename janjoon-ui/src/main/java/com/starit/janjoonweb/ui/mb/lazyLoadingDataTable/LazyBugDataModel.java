...
public class LazyBugDataModel extends LazyDataModel<JJBug> {
	...
	@Override
	public List<JJBug> load(int first, int pageSize, List<SortMeta> multiSortMeta, Map<String, Object> filters) {

		System.err.println("FIRST " + first);
		List<JJBug> data = new ArrayList<JJBug>();
		MutableInt size = new MutableInt(0);
		InputSwitch mineComponent = (InputSwitch) LoginBean.findComponent("mineBugs");
		...
		if (mineComponent.getValue() instanceof Boolean) {
			...
			if (mine != null && mine)
				contact = ((LoginBean) LoginBean.findBean("loginBean")).getContact();
			else if (mine == null) {
		...
		data = bugService.load(contact, LoginBean.getCompany(), size, first, pageSize, multiSortMeta, filters, project,
		        product, version);
		...
		((HttpSession) FacesContext.getCurrentInstance().getExternalContext().getSession(false)).setAttribute(
		        "bugDataTableOptions", new BugDataTableOptions(first, multiSortMeta, filters, contact != null));
