...
public class CategoryUtil  implements Serializable{
	...
	public static List<CategoryUtil> getCategoryList(List<JJCategory> categories,List<CategoryDataModel> tableDataModelList)
	{
		...
		for(JJCategory c:categories)
		{
			if(tableDataModelList != null)
			{
				...
				{
					checked=tableDataModelList.get(i).getCategoryId()==c.getId()&&tableDataModelList.get(i).getRendered();
					i++;
				}				
