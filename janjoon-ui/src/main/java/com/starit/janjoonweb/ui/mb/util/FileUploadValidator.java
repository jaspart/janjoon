	...
	public void validate(FacesContext context, UIComponent uiComponent,
		...
		if (fileName.length() == 0) {
			...
			FacesMessage facesMessage = MessageFactory
					.getMessage("validator_file_nameInvalid", "File");
			facesMessage.setSeverity(FacesMessage.SEVERITY_ERROR);
			throw new ValidatorException(facesMessage);
		} else if (fileName.length() > 50) {
			FacesMessage facesMessage = MessageFactory
					.getMessage("validator_file_nameLong", "File");
			facesMessage.setSeverity(FacesMessage.SEVERITY_ERROR);
			throw new ValidatorException(facesMessage);
		}
		...
		if (fileType.equalsIgnoreCase("image")) {
			if (!part.getContentType().contains("image")) {
				FacesMessage facesMessage = MessageFactory
						.getMessage("validator_file_TypeInvalid", "File");
				facesMessage.setSeverity(FacesMessage.SEVERITY_ERROR);
				throw new ValidatorException(facesMessage);
			}
		}
		...
		if (part.getSize() > 100000) {
			FacesMessage facesMessage = MessageFactory
					.getMessage("validator_file_sizeBig", "File");
			facesMessage.setSeverity(FacesMessage.SEVERITY_ERROR);
			throw new ValidatorException(facesMessage);
		}
	}
